...
//        char handsome : 1;
//    } _tallRichHandsome;
    // : 1 ==> 位域，只占1位的意思，声明的这3个成员就各占1bit，共3bit
    ...
    union {
        char bits;
        // 这个结构体纯属摆设，只为了提高可读性，说明bits里面存放的是这3个成员信息，并且每一个成员占1位
        // 而且这个结构体也只占1个字节（3个成员才各占1位），不会超出char的存储范围（1字节）
...
#define JPTallMask     (1<<0) // 左移0位 0b00000001 << 0 = 0b00000001 ==> 1
#define JPRichMask     (1<<1) // 左移1位 0b00000001 << 1 = 0b00000010 ==> 2
#define JPHandsomeMask (1<<2) // 左移2位 0b00000001 << 2 = 0b00000100 ==> 4

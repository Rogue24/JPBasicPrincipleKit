...
// 如果直接使用那都是0x0了，避免重复可以保存【自己的地址值】，因为地址值是唯一的。
// void *XXX --> 声明指针类型变量
// PS：用extern访问到这个全局变量
...
// PS1：指针（void *）在64bit中占【8个】字节，int占4个字节，char只占1个字节（char最小）
// PS2：加上`static`关键字可以防止外部通过`extern`访问到这个全局变量，只能【本文件内】可以访问
...
// key的用法【方式二】：直接使用字符串
// 直接写出来字符串（字面量）是放在常量区（代码区）的，所以内存地址是【唯一的】，同样的字符串内存只占一份
...
// key的用法【方式三】：直接使用属性get方法的地址（也可以是set或其他方法的地址，使用get方法地址是因为可读性更高）
// 方法的地址也是放在常量区（代码区）的，所以内存地址是【唯一的】
//【!!!推荐!!!】：不需要维护额外的key、可读性高
...
- (int)height {
    ...
    // 隐式参数：OC的方法默认会隐式传两个参数过来（没发现你可以在方法里面直接用到self吗？！）
    // 1：(id)self 方法调用者的指针
    // 2：(SEL)_cmd 方法选择器（当前方法的SEL）
}

// 可以给类/元类对象添加关联对象
+ (void)setMoney:(int)money {
    NSLog(@"setMoney %p", @selector(money));
    objc_setAssociatedObject(self, @selector(money), @(money), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
+ (int)money {
    NSLog(@"money %p", _cmd);
    return [objc_getAssociatedObject(self, _cmd) intValue];

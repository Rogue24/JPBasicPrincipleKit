 ...
 * 有，捕获的都是self这个局部变量
 * 【2】self为局部变量？
 * --> 从下面的方法编译的C++代码可以看得出：OC的方法默认会隐式传两个参数过来<(id)self 方法调用者的指针>和<(SEL)_cmd 方法选择器>
 * --> 所以self是一个参数，就是一个局部变量
 * --> 所以block捕获了self这个局部变量的指针，存储该地址值（JPPerson的实例对象）
 * 【3】为啥test2和test3捕获的也是self？
 * --> 因为 _name 相当于 self->_name，self.name 相当于 [self name]
 * --> 所以要捕获self，block内部才能访问self来获取这个值，毕竟这个值来自于self ==> JPPerson的实例对象
 * 【4】终极结论：block怎样才会捕获？

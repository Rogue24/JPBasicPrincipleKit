...
void test() {
    ...
    jpBlock = ^{
         ...
         * block将money的值捕获进来（capture）--> 值传递
            * money是自动变量（auto），离开作用域就销毁，之后就无法访问（不然就会坏内存访问，即野指针），所以auto进行的是值传递，捕获当前值
         * block将car的指针捕获进来（capture）--> 指针传递
            * car是静态变量（static），会一直保存在内存中，有地址就能一直访问（不然就只能在作用域内访问），所以static进行的是指针传递，捕获地址随时去访问最新值
         */
